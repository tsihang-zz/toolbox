...
#define vector_foreach_element(vector, foreach_element, element)\
	for (foreach_element = 0, element = NULL;\
		foreach_element <= (int)vector_active(vector); \
		element = (typeof(*element) *)vector_slot(vector, foreach_element), foreach_element++)
