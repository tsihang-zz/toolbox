 ...
 */
extern void vrf_add_hook (int, int (*)(uint16_t, void **));
...
extern vrf_iter_t vrf_iterator (uint16_t);

/*
 * VRF iterator to properties
 */
extern uint16_t vrf_iter2id (vrf_iter_t);
...
extern void *vrf_info_get (uint16_t);
/* Look up the data pointer of the specified VRF. */
extern void *vrf_info_lookup (uint16_t);
...
extern struct list *vrf_iflist (uint16_t);
/* Get the interface list of the specified VRF. Create one if not find. */
extern struct list *vrf_iflist_get (uint16_t);
...
extern void vrf_bitmap_set (vrf_bitmap_t, uint16_t);
extern void vrf_bitmap_unset (vrf_bitmap_t, uint16_t);
extern int vrf_bitmap_check (vrf_bitmap_t, uint16_t);
...
extern int vrf_socket (int, int, int, uint16_t);
