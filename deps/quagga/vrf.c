...
struct vrf_master
{
  int (*vrf_new_hook) (uint16_t, void **);
  int (*vrf_delete_hook) (uint16_t, void **);
  int (*vrf_enable_hook) (uint16_t, void **);
  int (*vrf_disable_hook) (uint16_t, void **);
} vrf_master = {0,};
...
void
vrf_add_hook (int type, int (*func)(uint16_t, void **))
{
  switch (type) {
  case VRF_NEW_HOOK:
...
void *
vrf_info_lookup (uint16_t vrf_id)
{
...
struct list *
vrf_iflist_get (uint16_t vrf_id)
{
   struct vrf * vrf = vrf_get (vrf_id);
   return vrf->iflist;
}
...
void
vrf_bitmap_set (vrf_bitmap_t bmap, uint16_t vrf_id)
{
  struct vrf_bitmap *bm = (struct vrf_bitmap *) bmap;
...
void
vrf_bitmap_unset (vrf_bitmap_t bmap, uint16_t vrf_id)
{
  struct vrf_bitmap *bm = (struct vrf_bitmap *) bmap;
  u_char group = VRF_BITMAP_GROUP (vrf_id);
  u_char offset = VRF_BITMAP_BIT_OFFSET (vrf_id);
...
int
vrf_bitmap_check (vrf_bitmap_t bmap, uint16_t vrf_id)
{
  struct vrf_bitmap *bm = (struct vrf_bitmap *) bmap;
  u_char group = VRF_BITMAP_GROUP (vrf_id);
  u_char offset = VRF_BITMAP_BIT_OFFSET (vrf_id);
...
int
vrf_socket (int domain, int type, int protocol, uint16_t vrf_id)
{
  struct vrf *vrf = vrf_lookup (vrf_id);
  int ret = -1;
