...
#define sse_memblock_size	1024000
...
void *lmemcpy(void *s1, const void *s2, size_t n)
{
	char *tmp = s1;
	const char *s = s2;
...
static __oryx_always_inline__
void elapsed(const char *prefix,
	void *(f)(void *s1, const void *s2, size_t n))
{
	...
	/* Prepare test data. */
	__src = malloc(sse_memblock_size);
	BUG_ON(__src == NULL);
	__dst = malloc(sse_memblock_size);
	BUG_ON(__dst == NULL);

	gettimeofday(&start,NULL);
    f(__dst, __src, sse_memblock_size);
	gettimeofday(&end, NULL);
	
	free(__src);
	free(__dst);

	fprintf (stdout, "%s: cost %lu usec\n", prefix, tm_elapsed_us(&start, &end));
}
    ...
    char __oryx_unused__ ** argv)
{
	...
	elapsed("lmemcpy", lmemcpy);
	elapsed("strncpy", strncpy);
	elapsed("memcpy", memcpy);
