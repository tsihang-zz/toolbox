#ifndef __BOX_RULE_PRIVATE_H__
#define __BOX_RULE_PRIVATE_H__
...
static __oryx_always_inline__
void appl_entry_lookup_id0 (vlib_appl_main_t *am, const uint32_t id, struct appl_t **appl)
{
				...
				struct appl_t **appl)
{
	...
	switch (lp->cmd) {
		case LOOKUP_ID:
			appl_entry_lookup_id0(am, (const) *((uint32_t *)lp->v), appl);
			break;
		case LOOKUP_ALIAS:
