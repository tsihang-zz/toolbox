...
BIN_NAME=ncapd
BIN=./src/x86_64-native-linuxapp-gcc/$BIN_NAME
...
proc_num() {
	n=`ps -ef | grep $1 | grep -v grep |grep -v "vi" | wc -l`
	return $n
}
...
dpdk_nap_monitor() {
	...
	then
		...
		log2 "$BIN_NAME is not running"
	fi
}

# if $BIN_NAME exist, this script will exit immediately.
# run StopNAPD.sh to stop NAP before you want restart it.
dpdk_nap_monitor
dpdk_nic_check
dpdk_env_check
dpdk_nic_bind
