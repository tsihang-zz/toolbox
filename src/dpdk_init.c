...
/* A tsc-based timer responsible for triggering statistics vty_out */
...

{
	...
	ret = l2fwd_parse_args(vm->argc, vm->argv);
	if (ret < 0)
		rte_exit(EXIT_FAILURE, "Invalid eal arguments\n");
	...
	dm->pktmbuf_pools = rte_pktmbuf_pool_create("mbuf_pool", conf->num_mbufs,
		conf->cache_size, conf->priv_size, conf->data_room_size,
		rte_socket_id());
#endif
	if (dm->pktmbuf_pools == NULL)
		rte_exit(EXIT_FAILURE, "Cannot init mbuf pool\n");

	dm->n_ports = nb_ports = rte_eth_dev_count();
	if (nb_ports == 0)
