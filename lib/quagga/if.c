...
struct interface *
if_create_vrf (const char *name, int namelen, uint16_t vrf_id)
{
...
struct interface *
if_lookup_by_index_vrf (ifindex_t ifindex, uint16_t vrf_id)
{
...
const char *
ifindex2ifname_vrf (ifindex_t ifindex, uint16_t vrf_id)
{
  struct interface *ifp;
...
ifindex_t
ifname2ifindex_vrf (const char *name, uint16_t vrf_id)
{
  struct interface *ifp;
...
struct interface *
if_lookup_by_name_vrf (const char *name, uint16_t vrf_id)
{
  struct listnode *node;
  struct interface *ifp;
...
struct interface *
if_lookup_by_name_len_vrf (const char *name, size_t namelen, uint16_t vrf_id)
{
  struct listnode *node;
  struct interface *ifp;
...
struct interface *
if_lookup_exact_address_vrf (struct in_addr src, uint16_t vrf_id)
{
  struct listnode *node;
...
/* Lookup interface by IPv4 address. */
struct interface *
if_lookup_address_vrf (struct in_addr src, uint16_t vrf_id)
{
  struct listnode *node;
  struct prefix addr;
  int bestlen = 0;
...
struct interface *
if_lookup_prefix_vrf (struct prefix *prefix, uint16_t vrf_id)
{
  struct listnode *node;
  struct listnode *cnode;
  struct interface *ifp;
...
/* Get interface by name if given name interface doesn't exist create
   one. */
struct interface *
if_get_by_name_vrf (const char *name, uint16_t vrf_id)
{
  struct interface *ifp;
...
struct interface *
if_get_by_name_len_vrf (const char *name, size_t namelen, uint16_t vrf_id)
{
  struct interface *ifp;
...
if_sunwzebra_get (const char *name, size_t nlen, uint16_t vrf_id)
{
  struct interface *ifp;
  size_t seppos = 0;
...
void
if_init (uint16_t vrf_id, struct list **intf_list)
{
  *intf_list = list_new ();
#if 0
...
void
if_terminate (uint16_t vrf_id, struct list **intf_list)
{
  for (;;)
    {
      struct interface *ifp;
