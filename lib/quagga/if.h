...
extern struct interface *if_create_vrf (const char *name, int namelen,
                                uint16_t vrf_id);
extern struct interface *if_lookup_by_index_vrf (ifindex_t, uint16_t vrf_id);
...
extern struct interface *if_lookup_by_name_len_vrf(const char *ifname,
                                size_t namelen, uint16_t vrf_id);
...
extern void if_init (uint16_t, struct list **);
extern void if_terminate (uint16_t, struct list **);
...
extern const char *ifindex2ifname_vrf (ifindex_t, uint16_t vrf_id);

/* Please use ifname2ifindex instead of if_nametoindex where possible;
   ifname2ifindex uses internal interface info, whereas if_nametoindex must
   make a system call. */
extern ifindex_t ifname2ifindex(const char *ifname);
extern ifindex_t ifname2ifindex_vrf(const char *ifname, uint16_t vrf_id);
