...
void
zclient_create_header (struct stream *s, uint16_t command, uint16_t vrf_id)
{
...
static int
zebra_message_send (struct zclient *zclient, int command, uint16_t vrf_id)
{
  struct stream *s;
...
void
zclient_send_requests (struct zclient *zclient, uint16_t vrf_id)
{
  int i;
...
struct interface *
zebra_interface_add_read (struct stream *s, uint16_t vrf_id)
{
 ...
 */
struct interface *
zebra_interface_state_read (struct stream *s, uint16_t vrf_id)
{
  struct interface *ifp;
  char ifname_tmp[INTERFACE_NAMSIZ];
...
struct connected *
zebra_interface_address_read (int type, struct stream *s, uint16_t vrf_id)
{
  ifindex_t ifindex;
  struct interface *ifp;
